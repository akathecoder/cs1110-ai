...
class snake(object):
    ...
    def moveAuto(
        self, key
    ):  # use this move method when directions are generated by successor method
    ...
    def getSuccessors(self, current_pos):  # more like surrounding grid positions
        ...
        successors = (
            []
        )  # tuple of states, actions, cost (grid pos, direction to get there, cost to get there)
        x, y = current_pos
        possible_moves = [
            -1,
            1,
        ]  # x or y can either stay, increase or decrease position by 1
                ...
                if nextState != current_pos:
                    if nextState not in successors:
                        ...
                        if movesX == 1:
                            directionX = "RIGHT"
                            cost = (
                                euclideanCost(current_pos, tempFood.pos) / 2
                            )  # prioritize left and right actions
                        elif movesX == -1:
                            directionX = "LEFT"
...
def main():
    ...
    while flag:
        ...
        if s.body[0].pos == snack.pos:
            ...
            snack = cube(randomSnack(rows, s), color=(0, 255, 0))
            tempFood = (
                food  # use this as testing to try to get food value before it changes
            )
...
def foodPos():
    ...
    for j in range(
        0, 399
    ):  # 400 grid positions, i.e. max num of food positions can be 400
        ...
        if current not in visited:
            ...
            for childNode, direction, cost in s.getSuccessors(current):
                if childNode not in aStar_priorityqueue.heap:
                    if (
                        childNode in visited
                    ):  # make sure child is not in visited so we don't go backwards
                        continue
                    hCost = costs + cost + manhattanHeuristic(childNode)
                    aStar_priorityqueue.push(
                        (childNode, directions + [direction], costs + cost), hCost
                    )
            ...
            for childNode, direction, cost in s.getSuccessors(current):
                if childNode not in ucs_priorityqueue.heap:
                    if (
                        ...
                        continue
                    ucs_priorityqueue.push(
                        (childNode, directions + [direction], costs + cost),
                        costs + cost,
                    )
...
def runSearch(run):
    ...
    my_file.write("{0:10}  {1:14}\n".format("RAW DFS SCORE:", scoreList[0]))
    my_file.write("{0:10}  {1:14}\n".format("RAW BFS SCORE:", scoreList[1]))
    my_file.write("{0:10}  {1:14}\n".format("RAW ASTAR SCORE:", scoreList[2]))
    my_file.write("{0:10}  {1:14}\n\n".format("RAW UCS SCORE:", scoreList[3]))

    my_file.write("{0:10}  {1:14}\n".format("CALC DFS SCORE:", calcScores[0]))
    my_file.write("{0:10}  {1:14}\n".format("CALC BFS SCORE:", calcScores[1]))
    ...
    dataFrame.plot(
        ...
        y="DFS",
        rot=70,
        title="Actions of Snake Game Search Algorithms "
        + str(run)
        + " "
        + str(datetime.now()),
    )
